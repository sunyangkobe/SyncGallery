...
public class Constants {
	...
	/** default image view maximum zoom. */
	public static final float IMAGE_VIEW_MAX_ZOOM = 8F;
	...
	private static void initContext( Context context ) {
		...
		if( ANDROID_SDK > 10 ){
			LARGE_HEAP = manager.getLargeMemoryClass() > MAX_MEMORY;
		}
		
		Log.i( LoggerFactory.LOG_TAG, "screen size: " + SCREEN_WIDTH + "x" + SCREEN_HEIGHT );
	}
	 ...
	 * Be careful if you want to modify the return value because
	 * it's easy to throw an {@link OutOfMemoryError} in android expecially when
	 * dealing with {@link Bitmap}.<br />
	 * Part of the application available memory has been already taken by the
	 * host application. 
	 * 
	...
	public static final int getManagedMaxImageSize() {
		...
		if ( MAX_MEMORY >= 48 ) {
			...
			return Math.min( screen_size, 800 );
		}
	}
	...
	/**
	 * If you want to enable the hi-res image post processing you need to pass a unique session id
	 * to the starting Intent. The session id string must be unique and must be 64 chars length
	 */

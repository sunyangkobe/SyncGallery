...
public class Gallery extends AbsSpinner implements GestureDetector.OnGestureListener {
	...
	/**
	 * The listener interface for receiving onItemsScroll events.
	 * The class that is interested in processing a onItemsScroll
	 * event implements this interface, and the object created
	 * with that class is registered with a component using the
	 * component's <code>addOnItemsScrollListener<code> method. When
	...
	public Gallery( Context context, AttributeSet attrs, int defStyle ) {
		 ...
		 * 
		 * int index = a.getInt(com.android.internal.R.styleable.Gallery_gravity, -1); if (index >= 0) { setGravity(index); }
		 * 
		 * int animationDuration = a.getInt(com.android.internal.R.styleable.Gallery_animationDuration, -1); if (animationDuration >
		 * 0) { setAnimationDuration(animationDuration); }
		 * 
		 * int spacing = a.getDimensionPixelOffset(com.android.internal.R.styleable.Gallery_spacing, 0); setSpacing(spacing);
		 * 
		 * float unselectedAlpha = a.getFloat( com.android.internal.R.styleable.Gallery_unselectedAlpha, 0.5f);
		 * setUnselectedAlpha(unselectedAlpha); a.recycle();
		 */
	 ...
	 *
	 * @param value the new on items scroll listener
	 */
	 ...
	 *
	 * @param value the new auto scroll to center
	 */
	 ...
	 *
	 * @param delta Change in the selected position. +1 means the selection is moving to the right, so views are scrolling to the left.
	...
	private View makeAndAddView( int position, int offset, int x, boolean fromLeft ) {
		...
		View child;
		final boolean valid = position >= 0 && position < mItemCount;
	...
	public boolean onFling( MotionEvent e1, MotionEvent e2, float velocityX, float velocityY ) {
		...
		// Fling the gallery!
		mFlingRunnable.startUsingVelocity( (int) -velocityX / 2 );
	...
	private void updateSelectedItemMetadata() {
		...
		// fire vibration
		if ( mSelectedPosition != mLastMotionValue ) {
			fireVibration();
		}
	 ...
	 */
	private class Fling8Runnable extends FlingRunnable {
		...
		/* (non-Javadoc)
		 * @see com.aviary.android.feather.widget.Gallery.FlingRunnable#springBack(int)
		 */
		@Override
		public void springBack( int delta ) {
			startUsingDistance( -delta );
		}

		/* (non-Javadoc)
		 * @see com.aviary.android.feather.widget.Gallery.FlingRunnable#_startUsingVelocity(int, int)
		 */
		@Override
		void _startUsingVelocity( int initialX, int initialVelocity ) {
			mScroller.fling( initialX, 0, initialVelocity, 0, 0, Integer.MAX_VALUE, 0, Integer.MAX_VALUE );
		}

		/* (non-Javadoc)
		 * @see com.aviary.android.feather.widget.Gallery.FlingRunnable#_startScroll(int)
		 */
		@Override
		void _startScroll( int distance ) {
			mScroller.startScroll( 0, 0, distance, 0, mAnimationDuration );
		}

		/* (non-Javadoc)
		 * @see com.aviary.android.feather.widget.Gallery.FlingRunnable#computeScrollOffset()
		 */
		@Override
		...
		/* (non-Javadoc)
		 * @see com.aviary.android.feather.widget.Gallery.FlingRunnable#getCurrX()
		 */
		@Override
		public int getCurrX() {
		...
		/* (non-Javadoc)
		 * @see com.aviary.android.feather.widget.Gallery.FlingRunnable#_endFling()
		 */
		@Override
		void _endFling() {
		...
		/* (non-Javadoc)
		 * @see com.aviary.android.feather.widget.Gallery.FlingRunnable#isFinished()
		 */
		@Override
	 ...
	 */
	private class Fling9Runnable extends FlingRunnable {
		...
		@Override
		public void springBack( int delta ) {
			mScroller.springBack( mLastFlingX, 0, 0, Integer.MAX_VALUE, 0, Integer.MAX_VALUE );
		}
		...
		@Override
		void _startUsingVelocity( int initialX, int initialVelocity ) {
			mScroller.fling( initialX, 0, initialVelocity, 0, 0, Integer.MAX_VALUE, 0, Integer.MAX_VALUE, 10, 1 );
		}
	...
	/**
	 * Responsible for fling behavior. Use {@link #startUsingVelocity(int)} to initiate a fling. Each frame of the fling is handled
	 * in {@link #run()}. A FlingRunnable will keep re-posting itself until the fling is done.
	 */
	private abstract class FlingRunnable implements Runnable {
		 ...
		 */
		abstract public void springBack( int delta );
		 ...
		 */
		abstract void _startUsingVelocity( int initialX, int initialVelocity );
		 ...
		 */
		abstract void _startScroll( int distance );

		/**
		 * _end fling.
		 */
		abstract void _endFling();
		...
		public void startUsingVelocity( int initialVelocity ) {
			...
			_startUsingVelocity( initialX, initialVelocity );
			post( this );
		}
		...
		public void run() {
			...
			boolean more = computeScrollOffset();
			final int x = getCurrX();

...
public class HighlightView {
 ...
 Grow
	...
	/** The internal_stroke_alpha_down. */
	private int internal_stroke_alpha, internal_stroke_alpha_down;
	...
	public HighlightView( View ctx ) {
		...
		internal_stroke_alpha = mContext.getResources().getInteger( R.integer.feather_crop_highlight_internal_stroke_alpha );
		internal_stroke_alpha_down = mContext.getResources().getInteger( R.integer.feather_crop_highlight_internal_stroke_alpha_down );
	}
	...
	protected void draw( Canvas canvas ) {
		...
		mInversePath.addRect( new RectF( mDrawRect ), Path.Direction.CCW );
		...
		float colStep = (float) mDrawRect.height() / cols;
		float rowStep = (float) mDrawRect.width() / rows;

		for ( int i = 1; i < cols; i++ ) {
			mLinesPath.moveTo( mDrawRect.left, mDrawRect.top + colStep * i );
			mLinesPath.lineTo( mDrawRect.right, mDrawRect.top + colStep * i );
		}

		for ( int i = 1; i < rows; i++ ) {
			mLinesPath.moveTo( mDrawRect.left + rowStep * i, mDrawRect.top );
			mLinesPath.lineTo( mDrawRect.left + rowStep * i, mDrawRect.bottom );
		}
	...
	public void setMode( Mode mode ) {
		if ( mode != mMode ) {
			...
			mOutlinePaint2.setColor( mMode == Mode.None ? highlight_color : highlight_down_color );
			mOutlinePaint2.setAlpha( mMode == Mode.None ? internal_stroke_alpha : internal_stroke_alpha_down );
	...
	public void setup( Matrix m, Rect imageRect, RectF cropRect, boolean maintainAspectRatio ) {
		...
		mOutlinePaint2.setAntiAlias( true );

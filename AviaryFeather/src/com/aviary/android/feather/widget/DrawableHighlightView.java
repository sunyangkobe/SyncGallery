...
public class DrawableHighlightView {
 ...
 Center
	...
	/**
	 * The listener interface for receiving onDeleteClick events.
	 * The class that is interested in processing a onDeleteClick
	 * event implements this interface, and the object created
	 * with that class is registered with a component using the
	 * component's <code>addOnDeleteClickListener<code> method. When
	 ...
	 *
	 * @param mode the new align mode v
	 */
	...
	protected void draw( final Canvas canvas ) {
		...
		if ( mSelected ) {
			...
			if ( mContent instanceof EditableDrawable ) {
				//( (EditableDrawable) mContent ).getFontMetrics( metrics );
			}

			mOutlinePath.reset();
			mOutlinePath.addRoundRect( drawRectF, mOutlineEllipse, mOutlineEllipse, Path.Direction.CW );

			if ( mDrawOutlineFill ) canvas.drawPath( mOutlinePath, mOutlineFillPaint );
			if ( mDrawOutlineStroke ) canvas.drawPath( mOutlinePath, mOutlineStrokePaint );
		}
	 ...
	 * @param dx the dx
	 * @param dy the dy
	 * @param checkMinSize the check min size
	 */
	protected void growBy( final float dx, final float dy, boolean checkMinSize ) {
		...
		if ( !mContent.validateSize( testRect ) && checkMinSize ) return;

		/*
		 * if (checkMinSize) { if ((r.width() < mMinWidth) || (r.height() < mMinHeight)) { final float diffw = mMinWidth - r.width();
		 * final float diffh = mMinHeight - r.height(); r.inset(-diffw, -diffh); Logger.error(this, "restored min sizes");
		 * 
	...
	private void init() {
		...
		mAnchorRotate = resources.getDrawable( R.drawable.feather_highlight_crop_handle );
		...
		mOutlineStrokeColor = mContext.getResources().getColor( R.color.feather_drawable_highlight_focus );
	 ...
	 * @param dx the dx
	 * @param dy the dy
	 * @param diffx the diffx
	 * @param diffy the diffy
	 */
	 ...
	 *
	 * @param value the new rotate and scale
	 */
	 ...
	 *
	 * @param color the new outline fill color
	 */
	 ...
	 *
	 * @param color the new outline fill color pressed
	 */
	 ...
	 *
	 * @param value the new outline ellipse
	 */
	 ...
	 *
	 * @param color the new outline stroke color
	 */
	 ...
	 *
	 * @param color the new outline stroke color pressed
	 */

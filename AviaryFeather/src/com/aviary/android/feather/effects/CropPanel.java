...
public class CropPanel extends AbstractContentPanel implements OnHighlightSingleTapUpConfirmedListener {
	...
	public void onActivate() {
		...
		createCropView( calculateAspectRatio( position, false ) );
	...
	private double calculateAspectRatio( int position, boolean inverted ) {
		...
		if ( values.length == 2 ) {
			...
			if ( aspectx == -1 ) {
				aspectx = inverted ? drawable.getIntrinsicHeight() : drawable.getIntrinsicWidth();
			}

			if ( aspecty == -1 ) {
				aspecty = inverted ? drawable.getIntrinsicWidth() : drawable.getIntrinsicHeight();
			}
	 ...
	 */
	private void createCropView( double aspectRatio ) {
		( (CropImageView) mImageView ).setImageBitmap( mBitmap, aspectRatio );
		//updateSelection( getOptionView().findViewById( R.id.button1 ) );
	}

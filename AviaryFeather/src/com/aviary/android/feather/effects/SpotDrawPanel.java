...
import com.aviary.android.feather.library.filters.PixelBrushFilter;
...
public class SpotDrawPanel extends AbstractContentPanel implements OnDrawListener {
	...
	public void onCreate( Bitmap bitmap ) {
		...
		mBrushSizes = config.getIntArray( R.array.feather_brush_sizes );
	...
	public void onDrawStart( float[] points, int radius ) {
		...
		mBackgroundDrawThread.setRadius( (float) radius, mPreview.getWidth() );
	...
	class MyHandlerThread extends Thread {
		...
		public void pause() {
			...
			boolean stopped = ( (PixelBrushFilter) mFilter ).stop();
		...
		public void setRadius( float radius, int bitmapWidth ) {
			( (PixelBrushFilter) mFilter ).setRadius( radius / bitmapWidth );
			mRadius = radius;
		}
		...
		public void run() {
			...
			while ( running ) {
				...
				if ( currentSize > 0 && !isInterrupted() ) {
					...
					if ( length == 0 ) {
						( (PixelBrushFilter) mFilter ).draw( firstPoint.x / mPreview.getWidth(), firstPoint.y / mPreview.getHeight(),
								mPreview );
						try {
							mActions.add( (MoaAction) ( (PixelBrushFilter) mFilter ).getActions().get( 0 ).clone() );
						...
						while ( currentPosition < length ) {
							...
							( (PixelBrushFilter) mFilter ).draw( point.x / mPreview.getWidth(), point.y / mPreview.getHeight(), mPreview );
							try {
								mActions.add( (MoaAction) ( (PixelBrushFilter) mFilter ).getActions().get( 0 ).clone() );
							} catch ( CloneNotSupportedException e ) {
								// TODO Auto-generated catch block

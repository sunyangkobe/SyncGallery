...
public class NativeEffectsPanel extends AbstractContentPanel implements ViewFactory, OnUpdateListener {
	...
	public void onCreate( Bitmap bitmap ) {
		...
		mLogger.error( "cpu: " + SystemUtils.getCpuSpeed() );
		...
		if ( enableFastPreview ) {
			...
			mSmallPreview = Bitmap.createBitmap( mBitmap.getWidth() / 3, mBitmap.getHeight() / 3, Config.ARGB_8888 );
		} else {
	...
	private View.OnClickListener mCannisterOnClickListener = new View.OnClickListener() {
		...
		public void onClick( View clickView ) {
			...
			if ( manager == null ) {
				onGenericError( "There was an error loading the effects for this pack" );
				return;
			}
			...
			trackPackage( appInfo.getApplicationInfo().packageName );
	...
	class FiltersPacksAdapter extends ArrayAdapter<FeatherPack> {
		...
		public FiltersPacksAdapter( Context context, int resource, int textViewResourceId, FeatherPack objects[] ) {
			...
			mLayoutInflater = UIUtils.getLayoutInflater();
			mGetMoreIcon = getContext().getResources().getDrawable( R.drawable.ic_filters_more );
				...
				if ( index < (count) ) {
					...
					if ( appInfo == null ) {
						...
						if( appInfo instanceof FeatherInternalPack )
							...
							icon = getContext().getResources().getDrawable(R.drawable.ic_filters_more);
	 ...
	 */
	private boolean installEffects( ResourceManager manager ) {

		if ( manager.isExternal() ) {

			final String name = manager.getPackageName();

			if ( EffectsPluginUtils.installed( name ) ) {
				return true;
			}

			return EffectsPluginUtils.addSharedLibrary( getContext().getBaseContext(), name );
		} else {
			return true;
		}
	}

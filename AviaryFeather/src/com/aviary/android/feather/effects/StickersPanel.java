...
public class StickersPanel extends AbstractContentPanel implements OnUpdateListener {
	...
	public void onCreate( Bitmap bitmap ) {
		...
		if( !mExternalPacksEnabled ){
			...
			setCurrentPack( mPluginService.new FeatherInternalPack( PackageManagerUtils.getApplicationInfo( getContext().getBaseContext() ) ) );
		}
	}
	...
	private void onApplyCurrent( boolean updateStatus ) {
		...
		if ( hv != null ) {
			...
			if ( mCurrentAction != null ) {
				...
				mCurrentAction.setValue( "rotation", hv.getRotation() );
	...
	private void addSticker( String drawable ) {
		...
		if ( stream != null ) {
			...
			if ( info != null ) {
				mCurrentAction = MoaActionFactory.action( "sticker" );
				mCurrentAction.setValue( "source", info.sourceDir );
				mCurrentAction.setValue( "name", drawable );
			}
		}
	}
	...
	private void addSticker( FeatherDrawable drawable, boolean rotateAndResize ) {
		...
		int cropWidth = drawable.getIntrinsicWidth();
		int cropHeight = drawable.getIntrinsicHeight();
				...
				if ( ( index + i ) < count ) {
					...
					if ( appInfo == null ) {
						...
						if ( appInfo instanceof FeatherInternalPack ) {
							...
							icon = getContext().getResources().getDrawable( R.drawable.feather_sticker_pack_background_more );
					}
